...
#define MV_UNKNOW_DEVICE                                                       \
  0x00000000 ///< \~chinese 未知设备类型，保留意义       \~english Unknown
             ///< Device Type, Reserved
#define MV_GIGE_DEVICE                                                         \
  0x00000001 ///< \~chinese GigE设备                     \~english GigE Device
#define MV_1394_DEVICE                                                         \
  0x00000002 ///< \~chinese 1394-a/b 设备                \~english 1394-a/b
             ///< Device
#define MV_USB_DEVICE                                                          \
  0x00000004 ///< \~chinese USB 设备                     \~english USB Device
#define MV_CAMERALINK_DEVICE                                                   \
  0x00000008 ///< \~chinese CamLink设备                  \~english CamLink
             ///< Device
...
typedef struct _MV_GIGE_DEVICE_INFO_ {
  ...
  unsigned int
      nIpCfgCurrent; ///< \~chinese       \~english IP
                     ///< configuration:bit31-static bit30-dhcp bit29-lla
  unsigned int nCurrentIp;
  unsigned int
      nCurrentSubNetMask; ///< \~chinese       \~english curtent subnet mask
  ...
  unsigned int
      nNetExport; ///< \~chinese 网口IP地址       \~english NetWork IP Address
...
typedef struct _MV_USB3_DEVICE_INFO_ {
  unsigned char CrtlInEndPoint; ///< \~chinese 控制输入端点         \~english
                                ///< Control input endpoint
  unsigned char CrtlOutEndPoint; ///< \~chinese 控制输出端点         \~english
                                 ///< Control output endpoint
  unsigned char StreamEndPoint; ///< \~chinese 流端点               \~english
                                ///< Flow endpoint
  unsigned char EventEndPoint; ///< \~chinese 事件端点             \~english
                               ///< Event endpoint
  unsigned short
      idVendor; ///< \~chinese 供应商ID号           \~english Vendor ID Number
  unsigned short
      idProduct; ///< \~chinese 产品ID号             \~english Device ID Number
  unsigned int nDeviceNumber; ///< \~chinese 设备序列号           \~english
                              ///< Device Serial Number
  unsigned char
      chDeviceGUID[INFO_MAX_BUFFER_SIZE]; ///< \~chinese 设备GUID号 \~english
                                          ///< Device GUID Number
  unsigned char chVendorName[INFO_MAX_BUFFER_SIZE]; ///< \~chinese 供应商名字
                                                    ///< \~english Vendor Name
  unsigned char chModelName[INFO_MAX_BUFFER_SIZE];  ///< \~chinese 型号名字
                                                    ///< \~english Model Name
  unsigned char chFamilyName[INFO_MAX_BUFFER_SIZE]; ///< \~chinese 家族名字
                                                    ///< \~english Family Name
  unsigned char
      chDeviceVersion[INFO_MAX_BUFFER_SIZE]; ///< \~chinese 设备版本号 \~english
                                             ///< Device Version
  unsigned char
      chManufacturerName[INFO_MAX_BUFFER_SIZE]; ///< \~chinese 制造商名字
                                                ///< \~english Manufacturer Name
  unsigned char
      chSerialNumber[INFO_MAX_BUFFER_SIZE]; ///< \~chinese 序列号 \~english
                                            ///< Serial Number
  unsigned char
      chUserDefinedName[INFO_MAX_BUFFER_SIZE]; ///< \~chinese 用户自定义名字
                                               ///< \~english User Defined Name
  unsigned int nbcdUSB; ///< \~chinese 支持的USB协议        \~english Support
                        ///< USB Protocol

  unsigned int
      nReserved[3]; ///< \~chinese 保留字节             \~english Reserved bytes
} MV_USB3_DEVICE_INFO;
...
typedef struct _MV_CC_DEVICE_INFO_ {
  ...
  unsigned int nTLayerType; ///< \~chinese 设备传输层协议类型，e.g.
                            ///< MV_GIGE_DEVICE\~english Device Transport Layer
...
typedef struct _MV_NETTRANS_INFO_ {
  int64_t
      nReviceDataSize; ///< \~chinese 已接收数据大小
                       ///< [统计StartGrabbing和StopGrabbing之间的数据量]\~english
                       ///< Received Data Size  [Calculate the Data Size
...
#define MV_MAX_TLS_NUM                                                         \
  8 ///< \~chinese 最多支持的传输层实例个数\~english The maximum number of
    ///< supported transport layer instances
#define MV_MAX_DEVICE_NUM                                                      \
  256 ///< \~chinese 最大支持的设备个数\~english The maximum number of supported
      ///< devices
...
typedef struct _MV_CC_DEVICE_INFO_LIST_ {
  unsigned int
      nDeviceNum; ///< \~chinese 在线设备数量\~english Online Device Number
  MV_CC_DEVICE_INFO
  *pDeviceInfo[MV_MAX_DEVICE_NUM]; ///< \~chinese 支持最多256个设备\~english
...
typedef struct _MV_FRAME_OUT_INFO_ {
  ...
  enum MvGvspPixelType
      enPixelType; ///< \~chinese 像素格式         \~english Pixel Type

  unsigned int nFrameNum; ///< \~chinese 帧号             \~english Frame Number
  unsigned int nDevTimeStampHigh; ///< \~chinese 时间戳高32位\~english Timestamp
                                  ///< high 32 bits
  unsigned int nDevTimeStampLow; ///< \~chinese 时间戳低32位\~english Timestamp
                                 ///< low 32 bits
  unsigned int nReserved0; ///< \~chinese 保留，8字节对齐\~english Reserved,
                           ///< 8-byte aligned
  int64_t nHostTimeStamp; ///< \~chinese 主机生成的时间戳\~english
                          ///< Host-generated timestamp

  unsigned int nFrameLen;

  unsigned int nLostPacket; ///< \~chinese 本帧丢包数\~english Lost Pacekt
                            ///< Number In This Frame
...
typedef struct _MV_FRAME_OUT_INFO_EX_ {
  ...
  float fExposureTime;
  unsigned int
      nAverageBrightness; ///< \~chinese 平均亮度\~english Average brightness
...
/// \~chinese 图像结构体，输出图像指针地址及图像信息\~english Image Struct,
/// output the pointer of Image and the information of the specific image
typedef struct _MV_FRAME_OUT_ {
  unsigned char
      *pBufAddr; ///< \~chinese 图像指针地址\~english  pointer of image
  MV_FRAME_OUT_INFO_EX stFrameInfo; ///< \~chinese 图像信息\~english information
...
typedef struct _MV_DISPLAY_FRAME_INFO_ {
  ...
  enum MvGvspPixelType
      enPixelType; ///< \~chinese 像素格式\~english Pixel format
  unsigned int nRes[4];
...
typedef struct _MV_SAVE_IMAGE_PARAM_T_ {
  unsigned char
      *pData; ///< [IN]   \~chinese 输入数据缓存\~english Input Data Buffer
  unsigned int
      nDataLen; ///< [IN]   \~chinese 输入数据大小\~english Input Data Size
  enum MvGvspPixelType
      enPixelType; ///< [IN]    \~chinese 输入数据的像素格式\~english Input Data
                   ///< Pixel Format
  unsigned short nWidth;  ///< [IN]   \~chinese 图像宽\~english Image Width
  unsigned short nHeight; ///< [IN]    \~chinese 图像高\~english Image Height

  unsigned char *pImageBuffer; ///< [OUT]   \~chinese 输出图片缓存\~english
                               ///< Output Image Buffer
  unsigned int
      nImageLen; ///< [OUT]   \~chinese 输出图片大小\~english Output Image Size
  unsigned int nBufferSize; ///< [IN]    \~chinese 提供的输出缓冲区大小\~english
                            ///< Output buffer size provided
  enum MV_SAVE_IAMGE_TYPE
      enImageType; ///< [IN]     \~chinese 输出图片格式\~english Output Image
                   ///< Format
...
typedef struct _MV_SAVE_IMAGE_PARAM_T_EX_ {
  ...
  enum MV_SAVE_IAMGE_TYPE
      enImageType; ///< [IN]     \~chinese 输出图片格式\~english Output Image
                   ///< Format
  unsigned int
      nJpgQuality; ///< [IN]    \~chinese
                   ///< JPG编码质量(50-99]，其它格式无效\~english Encoding
                   ///< quality(50-99]，Other formats are invalid

  ///< [IN]    \~chinese Bayer格式转为RGB24的插值方法  0-最近邻 1-双线性
  ///< 2-Hamilton （如果传入其它值则默认为最近邻） [IN]    \~english
  ///< Interpolation method of convert Bayer to RGB24  0-nearest neighbour
  ///< 1-bilinearity 2-Hamilton
...
typedef struct _MV_PIXEL_CONVERT_PARAM_T_ {
  unsigned short
      nWidth; ///< [IN]     \~chinese 图像宽           \~english Width
  unsigned short nHeight; ///< [IN]     \~chinese 图像高\~english Height

  enum MvGvspPixelType
      enSrcPixelType; ///< [IN]     \~chinese 源像素格式\~english Source pixel
                      ///< format
  unsigned char
  ...
  enum MvGvspPixelType
      enDstPixelType; ///< [IN]     \~chinese 目标像素格式\~english Destination
                      ///< pixel format
  unsigned char *pDstBuffer; ///< [OUT]    \~chinese 输出数据缓存\~english
                             ///< Output data buffer
  unsigned int
      nDstLen; ///< [OUT]    \~chinese 输出数据大小\~english Output data size
  unsigned int
      nDstBufferSize; ///< [IN]     \~chinese 提供的输出缓冲区大小\~english
                      ///< Provided outbut buffer size
...
typedef struct _MV_CC_RECORD_PARAM_T_ {
  ...
  unsigned short
      nWidth; ///< [IN]     \~chinese 图像宽(指定目标参数时需为2的倍数)
  unsigned short
      nHeight; ///< [IN]     \~chinese 图像高(指定目标参数时需为2的倍数)
...
typedef enum _MV_GIGE_TRANSMISSION_TYPE_ {
  ...
  MV_GIGE_TRANSTYPE_MULTICAST =
      0x1, ///< \~chinese 表示组播\~english Multicast mode
  MV_GIGE_TRANSTYPE_LIMITEDBROADCAST =
      0x2, ///< \~chinese 表示局域网内广播，暂不支持\~english Limited broadcast
           ///< mode,not support
  MV_GIGE_TRANSTYPE_SUBNETBROADCAST =
      0x3, ///< \~chinese 表示子网内广播，暂不支持\~english Subnet broadcast
           ///< mode,not support
  MV_GIGE_TRANSTYPE_CAMERADEFINED =
      0x4, ///< \~chinese 表示从相机获取，暂不支持\~english Transtype from
           ///< camera,not support
  MV_GIGE_TRANSTYPE_UNICAST_DEFINED_PORT =
      0x5, ///< \~chinese 表示用户自定义应用端接收图像数据Port号\~english User
           ///< Defined Receive Data Port
  MV_GIGE_TRANSTYPE_UNICAST_WITHOUT_RECV =
      0x00010000, ///< \~chinese 表示设置了单播，但本实例不接收图像数据\~english
                  ///< Unicast without receive data
  MV_GIGE_TRANSTYPE_MULTICAST_WITHOUT_RECV =
      0x00010001, ///< \~chinese 表示组播模式，但本实例不接收图像数据\~english
                  ///< Multicast without receive data
...
#define MV_MATCH_TYPE_NET_DETECT                                               \
  0x00000001 ///< \~chinese 网络流量和丢包信息\~english Network traffic and
             ///< packet loss information
#define MV_MATCH_TYPE_USB_DETECT                                               \
  0x00000002 ///< \~chinese host接收到来自U3V设备的字节总数\~english The total
             ///< number of bytes host received from U3V device

/// \~chinese 某个节点对应的子节点个数最大值\~english The maximum number of
/// child nodes corresponding to a node
#define MV_MAX_XML_NODE_NUM_C 128

/// \~chinese 节点名称字符串最大长度\~english The maximum length of node name
/// string
...
/// \~chinese 节点描述字段最大长度\~english The maximum length of the node
/// description field
...
/// \~chinese 每个已经实现单元的名称长度\~english The length of the name of each
/// unit that has been implemented
#define MV_MAX_XML_SYMBOLIC_STRLEN_C 64

#define MV_MAX_XML_SYMBOLIC_NUM 64

/// \~chinese 全匹配的一种信息结构体\~english A fully matched information
/// structure
typedef struct _MV_ALL_MATCH_INFO_ {
  unsigned int nType; ///< \~chinese 需要输出的信息类型，e.g.
                      ///< MV_MATCH_TYPE_NET_DETECT\~english Information type
                      ///< need to output ,e.g. MV_MATCH_TYPE_NET_DETECT
  void *pInfo; ///< \~chinese 输出的信息缓存，由调用者分配\~english Output
               ///< information cache, which is allocated by the caller
  unsigned int
      nInfoSize; ///< \~chinese 信息缓存的大小\~english Information cache size

} MV_ALL_MATCH_INFO;

/// \~chinese 网络流量和丢包信息反馈结构体，对应类型为 MV_MATCH_TYPE_NET_DETECT
/// \~en:Network traffic and packet loss feedback structure, the corresponding
/// type is MV_MATCH_TYPE_NET_DETECT
typedef struct _MV_MATCH_INFO_NET_DETECT_ {
  int64_t
      nReviceDataSize; ///< \~chinese 已接收数据大小
                       ///< [统计StartGrabbing和StopGrabbing之间的数据量]\~english
                       ///< Received data size
  int64_t nLostPacketCount; ///< \~chinese 丢失的包数量\~english Number of
                            ///< packets lost
  unsigned int
      nLostFrameCount; ///< \~chinese 丢帧数量\~english Number of frames lost
  unsigned int nNetRecvFrameCount;   ///< \~chinese 保留\~english Reserved
  int64_t nRequestResendPacketCount; ///< \~chinese 请求重发包数
  int64_t nResendPacketCount;        ///< \~chinese 重发包数
} MV_MATCH_INFO_NET_DETECT;

/// \~chinese host收到从u3v设备端的总字节数，对应类型为
/// MV_MATCH_TYPE_USB_DETECT\~english The total number of bytes host received
/// from the u3v device side, the corresponding type is MV_MATCH_TYPE_USB_DETECT
typedef struct _MV_MATCH_INFO_USB_DETECT_ {
  int64_t
      nReviceDataSize; ///< \~chinese 已接收数据大小
                       ///< [统计OpenDevicce和CloseDevice之间的数据量]\~english
                       ///< Received data size
  unsigned int nRevicedFrameCount; ///< \~chinese 已收到的帧数\~english Number
                                   ///< of frames received
  unsigned int
      nErrorFrameCount; ///< \~chinese 错误帧数\~english Number of error frames
...
typedef struct _MV_IMAGE_BASIC_INFO_ {
  ...
  unsigned int
      enPixelType; ///< \~chinese 当前的像素格式\~english Current pixel format
  unsigned int nSupportedPixelFmtNum; ///< \~chinese 支持的像素格式种类\~english
                                      ///< Support pixel format
...
#define MV_EXCEPTION_VERSION_CHECK                                             \
  0x00008002 ///< \~chinese SDK与驱动版本不匹配\~english SDK does not match the
             ///< driver version
/// \~chinese 设备的访问模式\~english Device Access Mode
/// \~chinese 独占权限，其他APP只允许读CCP寄存器\~english Exclusive authority,
/// other APP is only allowed to read the CCP register
#define MV_ACCESS_Exclusive 1
/// \~chinese 可以从5模式下抢占权限，然后以独占权限打开\~english You can seize
/// the authority from the 5 mode, and then open with exclusive authority
#define MV_ACCESS_ExclusiveWithSwitch 2
/// \~chinese 控制权限，其他APP允许读所有寄存器\~english Control authority,
/// allows other APP reading all registers
#define MV_ACCESS_Control 3
/// \~chinese 可以从5的模式下抢占权限，然后以控制权限打开\~english You can seize
/// the authority from the 5 mode, and then open with control authority
#define MV_ACCESS_ControlWithSwitch 4
/// \~chinese 以可被抢占的控制权限打开\~english Open with seized control
/// authority
#define MV_ACCESS_ControlSwitchEnable 5
/// \~chinese 可以从5的模式下抢占权限，然后以可被抢占的控制权限打开\~english You
/// can seize the authority from the 5 mode, and then open with seized control
/// authority
#define MV_ACCESS_ControlSwitchEnableWithKey 6
/// \~chinese 读模式打开设备，适用于控制权限下\~english Open with read mode and
/// is available under control authority
...
/// \~chinese 每个节点对应的接口类型\~english Interface type corresponds to each
/// node
...
typedef struct _MV_TRANSMISSION_TYPE_T {
  MV_GIGE_TRANSMISSION_TYPE
  enTransmissionType; ///< \~chinese 传输模式\~english Transmission type
  unsigned int
      nDestIp; ///< \~chinese 目标IP，组播模式下有意义\~english Destination IP
  unsigned short nDestPort; ///< \~chinese 目标Port，组播模式下有意义\~english
                            ///< Destination port
...
typedef struct _MV_ACTION_CMD_INFO_T {
  ...
  unsigned int bActionTimeEnable; ///< \~chinese 只有设置成1时Action
                                  ///< Time才有效，非1时无效
...
typedef struct _MV_ACTION_CMD_RESULT_T {
  ...
  int nStatus; // 1.0x0000:success.
               ...
               // 3.0x8013:The device is not synchronized to a master clock to
               // be used as time reference. 4.0x8015:A device queue or packet
               // data has overflowed. 5.0x8016:The requested scheduled action
               // command was requested at a time that is already past.
...
typedef struct _MV_XML_NODE_FEATURE_ {
  ...
  enum MV_XML_Visibility
      enVisivility; ///< \~chinese 是否可见\~english Is visibility
  char strDescription
      [MV_MAX_XML_DISC_STRLEN_C]; ///< \~chinese 节点描述，目前暂不支持\~english
                                  ///< Node Description, NOT SUPPORT NOW
  char
      strDisplayName[MV_MAX_XML_NODE_STRLEN_C]; ///< \~chinese 显示名称\~english
                                                ///< Display Name
  char strName[MV_MAX_XML_NODE_STRLEN_C]; ///< \~chinese 节点名\~english Node
                                          ///< Name
...
typedef struct _MV_XML_FEATURE_Value_ {
  ...
  char strDescription
      [MV_MAX_XML_DISC_STRLEN_C]; ///< \~chinese节点描述 目前暂不支持\~english
                                  ///< Node Description, NOT SUPPORT NOW
  char
      strDisplayName[MV_MAX_XML_NODE_STRLEN_C]; ///< \~chinese 显示名称\~english
                                                ///< Display Name
...
typedef struct _MV_XML_FEATURE_Integer_ {
  ...
  enum MV_XML_AccessMode
      enAccessMode; ///< \~chinese 访问模式\~english Access Mode
  int bIsLocked; ///< \~chinese 是否锁定。0-否；1-是    目前暂不支持\~english
                 ///< Locked. 0-NO; 1-YES, NOT SUPPORT NOW
...
typedef struct _MV_XML_FEATURE_String_ {
  ...
  int bIsLocked; ///< \~chinese 是否锁定。0-否；1-是    目前暂不支持\~english
                 ///< Locked. 0-NO; 1-YES, NOT SUPPORT NOW
  char strValue[MV_MAX_XML_STRVALUE_STRLEN_C]; ///< \~chinese 当前值\~english
                                               ///< Current Value
...
typedef struct _MV_XML_FEATURE_Enumeration_ {
  ...
  char strCurrentSymbolic
      [MV_MAX_XML_SYMBOLIC_STRLEN_C]; ///< \~chinese 当前Symbolic索引\~english
...
typedef struct _MVCC_ENUMVALUE_T {
  ...
  unsigned int nSupportedNum; ///< \~chinese 数据的有效数据个数\~english Number
                              ///< of valid data
